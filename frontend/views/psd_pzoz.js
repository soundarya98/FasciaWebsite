...
socket.on('SleepStage', function (fulldata) {
    d3.select("#svg_pzoz").remove();
    var setup = function() {

        width = parseInt(d3.select("#chart-psd-pzoz").style("width"))- margin.left - margin.right;
        height = 250;

        var values = fulldata.psd_pzoz;
        ...
        area = d3.area()
            .x(function(d) { return x(d.Frequencies); })
            .y0(height)
            .y1(function(d) { return y(d.PSD); });

        // A line generator, for the dark stroke.
        line = d3.line()
            .x(function(d) { return x(d.Frequencies); })
            .y(function(d) { return y(d.PSD); });

        values = fulldata.psd_pzoz;
        ...
        var svg = d3.select("#chart-psd-pzoz").append("svg")
            .attr("id","svg_pzoz")
