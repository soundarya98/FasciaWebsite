let keysToColor = {
    ...
    ["EEG_FPZ_CZ_Slow Waves"]:"#00FFFF",
    ["EEG_FPZ_CZ_Grad"]:"#DE0098",
    ["EEG_PZ_OZ"]:"#000000",
    ["EEG_PZ_OZ_Spindle"]:"#ff7f0e",
    ["EEG_PZ_OZ_Slow Waves"]:"#00FFFF",
...
function redraw_signal(data, channel, clientWidth, clientHeight, slowWave, spindle, saliency, now) {
    d3.select("#" + "svg-" + channel).remove();
    var chartDiv = document.getElementById(channel);
    var svg = d3.select(chartDiv).append("svg").attr("id", "svg-" + channel);
    ...
    var line = d3.line()
        ...
        var keys_ =   [channel];
         if(spindle){
            keys_.push(channel + "_Spindle");
        }
        if(slowWave){
            keys_.push(channel + "_Slow Waves");
        }
        if(saliency){
             keys_.push(channel + "_Grad");
        }
    ...
    let keysToColor = {
    [channel]: "#000000",
    [channel + "_Spindle"]:"#ff7f0e",
    [channel + "_Slow Waves"]:"#00FFFF",
    [channel + "_Grad"]:"#DE0098",
}
...
//         // z = d3.scaleOrdinal(d3.schemeCategory10);
//
//     var z = d3.scaleOrdinal(["#1f77b4", "#DE0098", "#00FFFF"]);
//
